...
"""Yerbas P2P network half-a-node.
...
def yerbashash(s):
    return yerbas_hash.getPoWHash(s)
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(yerbashash(r))
            self.hash = encode(yerbashash(r)[::-1], 'hex_codec').decode('ascii')
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.debug('Connecting to Yerbas Node: %s:%d' % (self.dstaddr, self.dstport))
