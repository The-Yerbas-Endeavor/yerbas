...
// Copyright (c) 2020-2023 The Yerbas developers
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager &am, const std::string &arg) {
        ...
        // argument value seen from the command line (so "yerbasd -foo=bar
...
bool ArgsManager::ReadConfigFiles(std::string &error, bool ignore_invalid_keys) {
    ...
    if (stream.good()) {
        ...
        // Create an empty yerbas.conf if it does not exist
...
std::string CopyrightHolders(const std::string &strPrefix, unsigned int nStartYear, unsigned int nEndYear) {
    ...
    // Check for untranslated substitution to make sure Yerbas Core copyright is not removed by accident
    if (copyright_info.find("Yerbas") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2020, nEndYear) + "The Yerbas developers";
    }
