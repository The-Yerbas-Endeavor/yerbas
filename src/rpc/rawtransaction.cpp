...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3) {
        throw std::runtime_error(
            ...
            "   [\n"
            "    {\n"
            "      \"address\": x.xxx,    (obj, optional) A key-value pair. The key (string) is the yerbas address, the value (float or string) is the amount in " + CURRENCY_UNIT + "\n"
            "    },\n"
            "    {\n"
            "      \"address\":           (obj, optional) A key-value pair. The key (string) is the yerbas address, value is a json string, numberic pair for future_maturity, future_locktime, and future_amount\n"
            ...
            "           \"future_amount\":n    (numeric, required) yerbas amount to be locked\n"
            "         }\n"
            "    },\n"                                                                                                                                                                                                                                                                                                                                              "    },\n"
            "    {\n"
...
UniValue signrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("signrawtransaction")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "signrawtransaction is deprecated and will be fully removed in v0.18. "
            "To use signrawtransaction in v0.17, restart yerbasd with -deprecatedrpc=signrawtransaction.\n"
