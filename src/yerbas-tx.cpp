...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf("%s yerbas-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  yerbas-tx [options] <hex-tx> [commands]  Update hex-encoded yerbas transaction\n" +
              "  yerbas-tx [options] -create [commands]   Create hex-encoded yerbas transaction\n" +
              "\n";
